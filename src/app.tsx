...
export function App() {
  ...
  const [emailsToInvite, setEmailsToInvite] = useState([
    'lucas.lima@metalsa.com',"dani@metalsa.com"
  ])
  ...
  function addNewImailToInvite(event: FormEvent<HTMLFormElement>){
      ...
      <div className="max-w-3xl w-full px-6 text-center space-y-10">
        ...
        <div className="space-y-4">
          <div className="flex items-center h-16 px-4 rounded-xl bg-zinc-900 shadow-shape gap-3">
            ...
            {isGuestsInputOpen ? (
              <button onClick={closeGuestsInput} className="flex items-center gap-2 bg-zinc-800 text-zinc-200 text-sm rounded-lg px-5 py-2 h-9 hover:bg-zinc-700">
                Alterar local/data
                <Settings2 className="size-5" />
              </button>
            ) :(

              <button onClick={openGuestsInput} className="flex items-center gap-2 bg-lime-300 text-lime-950 text-sm rounded-lg px-5 py-2 h-9 hover:bg-lime-400">
                Continuar
          ...
          {isGuestsInputOpen && (
            <div className="flex items-center h-16 px-4 rounded-xl bg-zinc-900 shadow-shape gap-3">
              ...
              <button className="flex items-center gap-2 bg-lime-300 text-lime-950 text-sm rounded-lg px-5 py-2 h-9 hover:bg-lime-400">
                Confirmar viagem
                <ArrowRight className="size-5" />
              </button>
            </div>
      ...
      {isGuestsModalOpen && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center">
          <div className="w-[640px] rounded-xl py-5 px-5 shadow-shape bg-zinc-900 space-y-5">
            ...
            <form onSubmit={addNewImailToInvite} className="px-4 py-2.5 bg-zinc-950 border border-zinc-800 rounded-lg flex items-center gap-2">
              <div className="flex items-center gap-2 flex-1">
              ...
              <button type="submit" className="flex items-center gap-2 bg-lime-300 text-lime-950 text-sm rounded-lg px-5 py-2 h-9 hover:bg-lime-400">
              Convidar
